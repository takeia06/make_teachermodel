...
class QFormerRefCOCO(nn.Module):
        ...
        if isinstance(pretrained_qformer, str) and len(pretrained_qformer) > 0:
            try:
                ...
                for i, layer in enumerate(enc.layer):
                    ...
                    # fix typo: quary -> query
                    print(f"[probe layer.{i}: |Q|={_stat(ca.query):.6f} |K|={_stat(ca.key):.6f}]")

                # ---K/V実寸の強制検証 ---
                try:
                    expect = int(getattr(self.qformer.bert.config, "encoder_hidden_size",
                                 getattr(self.qformer.bert.config, "cross_attention_hidden_size", 1408)))
    ...
    def forward(self, inputs: Dict[str, Any], compute_lm: bool = False) -> Dict[str, Any]:
        ...
        probs = attn_maps.clamp(1e-6, 1-1e-6)
        attn_logits = torch.log(probs) - torch.log1p(-probs)
    ...
    # ---------------------------
    @staticmethod
    def _bbox_from_attn_map(attn_map: torch.Tensor,
                            area_floor_frac: float = 5e-4) -> List[float]:
        """
        attn_map: (1,1,H,W) or (H,W) torch, 値域は [0,1] を想定
        return: [x1,y1,x2,y2] in [0,1] （H,W 基準）
        """
        if attn_map.dim() == 4:
            m = attn_map[0, 0].detach().float().cpu().numpy()
        elif attn_map.dim() == 2:
            m = attn_map.detach().float().cpu().numpy()
        else:
            raise ValueError("attn_map must be (1,1,H,W) or (H,W)")
        ...
        _, binm = cv2.threshold(m_u8, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
        ...
        if not cnts:
            ...
            # あまりに小さい場合は argmax 周辺の最小枠
            if (w*h) < (area_floor_frac * H * W):
                y, x = np.unravel_index(np.argmax(m), m.shape)
                x1 = max(0, x-2); y1 = max(0, y-2); x2 = min(W-1, x+2); y2 = min(H-1, y+2)
            else:
    ...
    @staticmethod
    def _clip_and_pad_bbox(bb: List[float]) -> List[float]:
    ...
    def predict(self,
                ...
                return_attn_maps: bool = True,
                return_attn_logits: bool = True,
                return_bbox: bool = True,
                compute_lm: bool = False,
                **kwargs) -> Dict[str, Any]:
        """
        受け取り方は柔軟に（samples or (image,text)）。
        返り値に "bbox" を必ず含める。
        """
        if samples is None:
            samples = {}
            if image is not None: samples["image"] = image
            if text  is not None: samples["text"]  = text
        assert "image" in samples, "samples['image'] is required (B,3,H,W) tensor"

        out = self.forward(samples, compute_lm=compute_lm)
        # 確率マップ（0-1）を優先利用
        attn = out.get("attn_maps", None)
        if not isinstance(attn, torch.Tensor):
            attn = out.get("attn_logits", None)
            attn = torch.sigmoid(attn) if isinstance(attn, torch.Tensor) else None

        B = samples["image"].shape[0]
        bboxes: List[List[float]] = []
        area_cap = float(os.getenv("EVAL_AREA_CAP", "0.85"))
        for b in range(B):
            if isinstance(attn, torch.Tensor):
                bb = self._bbox_from_attn_map(attn[b:b+1], area_floor_frac=5e-4)
            else:
                # 万一 attn が無い場合は全画面
                bb = [0.0,0.0,1.0,1.0]
            bb = self._clip_and_pad_bbox(bb)
            # 面積ガード（大きすぎるときは argmax 最小枠を採用し直す）
            w = max(0.0, bb[2]-bb[0]); h = max(0.0, bb[3]-bb[1])
            if (w*h) >= area_cap and isinstance(attn, torch.Tensor):
                bb = self._bbox_from_attn_map(attn[b:b+1], area_floor_frac=5e-4)
                bb = self._clip_and_pad_bbox(bb)
            bboxes.append(bb)
        ...
        if return_bbox:
            ret["bbox"] = bboxes
        if return_attn_logits and "attn_logits" in out:
            ret["attn_logits"] = out["attn_logits"]
        if return_attn_maps and "attn_maps" in out:
            ret["attn_maps"] = out["attn_maps"]
        # 参考情報もそのまま返す（eval 側が拾うことがある）
        ret["feat_hw"] = out.get("feat_hw")
        ret["vision_emb"] = out.get("vision_emb")
        ret["attn_logits_sim"] = out.get("attn_logits_sim")
        return ret
