# code/trainers/train_refcoco_inst.py
import os
import time
import argparse
import yaml
...
def build_optimizer(model: nn.Module, cfg: Dict[str, Any]) -> torch.optim.Optimizer:
    ...
    print(f"[sanity] pg_head={len(pg_head)} | pg_q_last={len(pg_q_last)} | pg_q_kv={len(pg_q_kv)}")
    return optim
...
def train(cfg: Dict[str, Any]):
    ...
    for ep in range(1, epochs + 1):
        ...
        for it, batch in enumerate(dl_tr, start=1):
            ...
            # 追加：OpenCLIPに入れる前だけ正方に揃える（学習/評価の両方で）
            import torch.nn.functional as F
        ...
        dt = time.time() - t0
        print(f"[Epoch {ep}] val_loss={val_loss:.4f} | val_iou@p(cur)={val_iou_q:.3f} | val_iou@0.35={val_iou_035:.3f} | val_iou@0.5={val_iou_05:.3f} | val_iou@0.75={val_iou_075:.3f}  ({dt:.1f}s)")
        ...
        writer.add_scalar("val/prob_mean", float(probs.mean().item()), ep)
        writer.add_scalar("val/prob_pos_mean", float(probs[gt>=0.5].mean().item()), ep)
        writer.add_scalar("val/prob_neg_mean", float(probs[gt<0.5].mean().item()), ep)

        _csv_append(epoch_csv, epoch_row, field_order=["epoch","val_loss","val_iou_p","val_iou_035","val_iou_05","val_iou_075","elapsed_sec"])
